    ...
    def __init__(
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                ...
                pad_input = tf.pad(self.embedded_chars_expanded, [[0, 0], [1, filter_size - 2], [0, 0], [0, 0]],
                                   mode='CONSTANT')
                ...
                pooled = tf.nn.avg_pool(
                    self.h,
                    ksize=[1, 100, 1, 1],  # sequence_length - filter_size + 1
                    strides=[1, 1, 1, 1],
                    padding='VALID',
                    name="pool")
...
for num, batch in enumerate(batches):
    ...
    if len(x_batch) == FLAGS.batch_size:
        ...
        for batch_idx in range(FLAGS.batch_size):
            ...
            preinfo = {'실제값': y_train[doc_idx], '예측값': predictions[batch_idx]}
...
for result_idx, data in enumerate(results[3:]):
    ...
    if data[0]['예측값'] == 0:
        for num in range(len(tmp)):
            if type(tmp[num]) == int:
                continue
            positive_dict[tmp[num][:3]] += 1
    else:
        for num in range(len(tmp)):
            if type(tmp[num]) == int:
                continue
            negative_dict[tmp[num][:3]] += 1
...
'''
# extract weights and activation maps
filter_size = list(map(int, FLAGS.filter_sizes.split(",")))
# weights 차원수 [activation map=num_filters 개수 X class 개수] = [384 X 2]
weights = sess.run(cnn.finW)
results = []
sents = []
n = 0
result_batches = batch_iter(list(zip(x_train, y_train)), FLAGS.batch_size, 1)
for result_batch in result_batches:
    while n < 10:
        result_x_batch, result_y_batch = zip(*result_batch)
        # activations 차원수
        # [batch_size, activation map 높이, activation 너비(1), activation map 개수=num_filter=128]
        activations, predictions = sess.run([cnn.h_outputs, cnn.predictions],
                                            feed_dict={cnn.input_x: result_x_batch,
                                                       cnn.dropout_keep_prob: 1.0})
        result = np.zeros((len(predictions), max_document_length))
        for data_idx in range(len(predictions)):
            filter_ = 0
            for i, filter_height in enumerate(filter_size):
                actmap_height = max_document_length - filter_height + 1
                tmp = activations[i][data_idx].reshape((actmap_height, FLAGS.num_filters))
                for actmap_idx in range(actmap_height):
                    result[data_idx, actmap_idx:(actmap_idx + filter_height - 1)] += \
                        tmp[i, actmap_idx] * weights[(actmap_idx + filter_), predictions[data_idx]]
                filter_ += FLAGS.num_filters
        results.append(result)
        for i in range(len(predictions)):
            sent = [vocabulary[word] for word in result_x_batch[i]]
            sents.append(sent)
        n += 1
...
for result in results:
    for i in range(len(result)):
        print(sents[sent_id][np.argmax(result[i])])
        sent_id += 1
'''
