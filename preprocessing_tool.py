...
# cut words function                               #
####################################################
def cut(contents, points, cut=2, threshold=10):
    results_con = []
    results_pt = []
    for idx, content in enumerate(contents):
        ...
        if len(words) > threshold:
            ...
            results_con.append(' '.join([token for token in result]))
            results_pt.append(points[idx])
    return results_con, results_pt
# contents, importants = tool.cut(corpus, importants, cut=2, threshold=50)
####################################################
...
####################################################
def make_output(importants): # 얘 수정필요함. 이건 왓챠 평점이고 기사같은경우는 어떻게?
    results = np.zeros((len(importants),2))
    for idx, point in enumerate(importants):
        if point is 1:
            results[idx,0] = 1
        else:
            results[idx,1] = 1
...
# loading function                                 #
####################################################
def loading_rdata(data_path, eng=True, num=True, punc=False): #얘도수정. 나는 바로 불러온다. 전처리도 여기서 끝내자
    ...
    for idx,doc in enumerate(corpus):
        if isNumber(doc[0]) is False:
            ...
            importants.append(doc[1])
        if idx % 100000 is 0:
            print('%d docs / %d save' % (idx, len(contents)))
    return contents, importants
...
class CohesionProbability:
    def __init__(self, left_min_length=1, left_max_length=10,
                 right_min_length=1, right_max_length=6):
